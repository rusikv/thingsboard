...
export class AlarmsTableWidgetComponent extends PageComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private initializeConfig() {
    ...
    this.columnsWithCellClick = this.ctx.actionsApi.getActionDescriptors('cellClick')
      .map(action => action.columnInfo?._hash);
    let enableFilter;
  ...
  public onCellClick($event: Event, alarm: AlarmDataInfo, column: EntityColumn) {
  ...
  public columnHasCellClick(hash: number) {
    if (this.columnsWithCellClick.length) {
      return this.columnsWithCellClick.includes(hash);
    }
  }
